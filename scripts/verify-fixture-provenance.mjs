...
for (const artifact of provenance.artifacts) {
  const content = execFileSync(
    "git",
    ["-C", protocolDir, "show", `${provenance.source.commit}:${artifact.path}`],
    { encoding: "utf8" }
  );
  ...
  sourceArtifacts.set(artifact.path, content);
}
...
for (const binding of provenance.fixtureBindings) {
  if (!fixtures.has(binding.fixture)) {
    ...
    if (fixture.sourceCommit !== provenance.source.commit) {
      throw new Error(`Fixture source commit mismatch for ${binding.fixture}`);
    }
    fixtures.set(binding.fixture, fixture);
  }
  const sourceContent = sourceArtifacts.get(binding.sourceArtifact);
  if (!sourceContent) throw new Error(`Fixture binding references an unpinned source artifact: ${binding.sourceArtifact}`);
  ...
  const actual = atPath(fixtures.get(binding.fixture), binding.fixturePath, `fixture:${binding.fixture}`);
